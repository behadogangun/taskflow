...
export function useBoard(initialColumns: ColumnWithCards[], boardId: string) {
  ...
  const logActivity = async (cardId: string, action: string, detail?: string) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (draggedCard && originalSourceColId && originalSourceColId !== targetCol.id) {
      ...
      await logActivity(activeId, 'moved', `"${draggedCard.title}" kartı "${sourceColTitle}" → "${targetCol.title}" sütununa taşındı`)
    }
  }
  ...
  const deleteColumn = async (columnId: string): Promise<void> => {
    ...
    if (error) { showToast('Sütun silinirken bir hata oluştu.', 'error'); return }
    setColumns(prev => prev.filter(c => c.id !== columnId))
    showToast('Sütun silindi.', 'delete')
  }

  const updateColumnTitle = async (columnId: string, title: string): Promise<void> => {
    const { error } = await supabase.from('columns').update({ title: title.trim() }).eq('id', columnId)
    if (error) { showToast('Sütun başlığı güncellenirken hata oluştu.', 'error'); return }
  ...
  const deleteCard = async (cardId: string, columnId: string): Promise<void> => {
    ...
    if (error) { showToast('Kart silinirken bir hata oluştu.', 'error'); return }
  ...
  const updateCard = async (cardId: string, columnId: string, title: string, description: string, priority: Priority, dueDate: string | null, assignee: string | null, labels: string[]): Promise<void> => {
    ...
    setColumns(prev => prev.map(col =>
      col.id === columnId ? { ...col, cards: col.cards.map(c => c.id === cardId ? { ...c, title: title.trim(), description: description.trim(), priority, due_date: dueDate, assignee, labels } : c) } : col
    ))
    showToast('Kart güncellendi.', 'success')
    await logActivity(cardId, 'updated', `"${title.trim()}" kartı güncellendi`)
  }

  const toggleComplete = async (cardId: string, columnId: string, completed: boolean, completedBy: string): Promise<void> => {
    const { error } = await supabase.from('cards').update({ completed, completed_by: completed ? completedBy : null }).eq('id', cardId)
    if (error) { showToast('Kart güncellenirken hata oluştu.', 'error'); return }
    const card = columns.flatMap(c => c.cards).find(c => c.id === cardId)
    setColumns(prev => prev.map(col =>
      col.id === columnId ? { ...col, cards: col.cards.map(c => c.id === cardId ? { ...c, completed, completed_by: completed ? completedBy : null } : c) } : col
    ))
    await logActivity(cardId, completed ? 'completed' : 'uncompleted',
      completed ? `"${card?.title}" kartı ${completedBy} tarafından tamamlandı` : `"${card?.title}" kartı tamamlanmadı olarak işaretlendi`
    )
  }
